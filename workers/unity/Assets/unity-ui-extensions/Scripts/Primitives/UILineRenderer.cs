    ...
    public class UILineRenderer : UIPrimitiveBase
	{
		...
		private void PopulateMesh(VertexHelper vh, Vector2[] pointsToDraw)
		{
			...
			if (lineList) {
				for (var i = 1; i < pointsToDraw.Length; i += 2) {
					...
					segments.Add (CreateLineSegment (start, end, SegmentType.Middle, i));
			...
			for (var i = 0; i < segments.Count; i++) {
                ...
                /*
                Debug.Log("UIVERTCOUNT: " + UIVerteciesList.Count);

                for (var l = 0; l < UIVerteciesList.Count; l++)
                {
                    var v = UIVerteciesList[l];
                    v.color = Color.green;
                    UIVerteciesList[l] = v;
                }
                */
		...
		private UIVertex[] CreateLineCap(Vector2 start, Vector2 end, SegmentType type)
		{
			if (type == SegmentType.Start)
			{
				...
				return CreateLineSegment(capStart, start, SegmentType.Start, 0);
			}
			else if (type == SegmentType.End)
			{
				var capEnd = end + ((end - start).normalized * lineThickness / 2);
				return CreateLineSegment(end, capEnd, SegmentType.End, 0);
			}
		...
		private UIVertex[] CreateLineSegment(Vector2 start, Vector2 end, SegmentType type, int segmentIndex)
		{
            ...
            switch (type)
            {
                case SegmentType.Start:
                    return SetVbo(new[] { v1, v2, v3, v4 }, startUvs, segmentIndex);
                case SegmentType.End:
                    return SetVbo(new[] { v1, v2, v3, v4 }, endUvs, segmentIndex);
                case SegmentType.Full:
                    return SetVbo(new[] { v1, v2, v3, v4 }, fullUvs, segmentIndex);
                default:
                    return SetVbo(new[] { v1, v2, v3, v4 }, middleUvs, segmentIndex);
            }
		}

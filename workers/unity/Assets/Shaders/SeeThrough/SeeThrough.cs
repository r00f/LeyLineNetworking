...
class SeeThrough : CustomPass
{
    ...
    protected override void AggregateCullingParameters(ref ScriptableCullingParameters cullingParameters, HDCamera camera) {
        ...
        cullingParameters.cullingMask = cu

    }
    */

    protected override void Execute(ScriptableRenderContext ctx, CommandBuffer buff, HDCamera cam, CullingResults cull)
    {
        ...
        RenderObjects(ctx, buff, stencilMaterial, 0, CompareFunction.LessEqual, cull, cam);
        ...
        RenderObjects(ctx, buff, seeThroughMaterial, seeThroughMaterial.FindPass("ForwardOnly"), CompareFunction.GreaterEqual, cull, cam, seeThroughStencil);
    }

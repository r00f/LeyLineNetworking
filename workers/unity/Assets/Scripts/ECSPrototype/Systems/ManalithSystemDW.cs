...
namespace LeyLineHybridECS
{
    ...
    public class ManalithSystemDW : JobComponentSystem
    {
        UISystem m_UISystem;
        ...
        private void InitWorlds()
        {
            ...
            {
                Debug.Log("InitManalithSystemDW");
        ...
        {
            base.OnStartRunning();
            Entities.WithStoreEntityQueryInField(ref m_CircleData).ForEach((MeshColor meshColor) =>
            {
        ...
        protected override JobHandle OnUpdate(JobHandle inputDeps)
        {
            ...
            var gameState = m_GameControllerData.GetSingleton<GameState.Component>();
            ...
            if (initMapEvents.Count > 0)
            {
                ...
                for (int i = 0; i < manaLithFactions.Length; i++)
                {
                    ...
                    Entities.WithStoreEntityQueryInField(ref m_CircleData).ForEach((Entity e, ManalithClientData clientData, MeshColor meshColor, ManalithInitializer initData/*, StudioEventEmitter eventEmitter*/) =>
                    {
                        ...
                        for (int y = 0; y < manalithUnitEntittyIds.Length; y++)
                        {
                            ...
                            {
                                clientData.ManalithUnitTeamColorMeshes = manalithUnitTeamColorMeshes[y];
                            }
                        }
            ...
            for (int i = 0; i < manaLithFactions.Length; i++)
            {
                ...
                Entities.WithStoreEntityQueryInField(ref m_CircleData).ForEach((Entity e, ManalithClientData clientData, MeshColor meshColor, ManalithInitializer initData, StudioEventEmitter eventEmitter) =>
                {
                    if (clientData.ManalithEntityID == ID)
                    {
                        ...
                        {
                            if (Vector3fext.ToUnityVector(manaLith.ManalithUnitCoordinate) == Vector3fext.ToUnityVector(playerHigh.HoveredCoordinate))
                            {
            ...
            Entities.WithStoreEntityQueryInField(ref m_CircleData).ForEach((MeshColor meshColor) =>
            {
                ...
                {
                    meshColor.ManaLithObject.DetailColorRenderers[i].sharedMaterial.SetColor("_BaseColor1", meshColor.LerpColor);
                }
            ...
            Entities.WithStoreEntityQueryInField(ref m_LineData).ForEach((MeshGradientColor meshGradientColor, ManalithClientData clientData) =>
            {
